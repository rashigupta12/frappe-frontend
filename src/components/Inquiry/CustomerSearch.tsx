...
const CustomerSearch: React.FC<CustomerSearchProps> = ({
  ...
  const handleOpenEditModal = () => {
    ...
    setCustomerForm({
      ...
      jobType:
        selectedCustomer.custom_jobtype || formData?.custom_jobtype || [],
...
const handleSaveCustomer = async () => {
  ...
  try {
    ...
    // Convert jobType array to the required format for child table
    const jobTypeData = customerForm.jobType.map(jobType => ({
      job_type: jobType // This creates the proper object structure
    }));
    ...
    const customerData = {
      ...
      custom_jobtype: jobTypeData, // Use the properly formatted array
      ...
      custom_special_requirements: formData?.custom_special_requirements || "",
      custom_reference_name: formData?.custom_reference_name || "",
    };

    console.log("Sending payload:", JSON.stringify(customerData, null, 2)); // Debug log

    let result;
    if (modalMode === "create") {
      result = await createLead(customerData);
    } else {
      result = await updateLead(customerForm.lead_id, customerData);
      result = { ...customerData, name: customerForm.lead_id };
    }

    if (!result) {
      throw new Error(`Failed to ${modalMode} customer`);
    }
    ...
    const updatedCustomer: CustomerData = {
      ...
      custom_jobtype: customerForm.jobType, // Keep as simple array for local state
    };
    ...
    toast.error(`Failed to ${modalMode} customer. Please try again.`);
  } finally {
            ...
            <div className="space-y-2">
              ...
              <MultiSelectJobTypes
                ...
                onSelectionChange={(selected) => {
                  setCustomerForm((prev) => ({ ...prev, jobType: selected }));
                }}
