...
const PropertyAddressSection: React.FC<PropertyAddressSectionProps> = ({
  ...
  const debouncedSearch = useCallback(
    debounce(async (query: string) => {
      ...
      try {
        const response = await frappeAPI.get(`/api/method/eits_app.area_search.search_uae_areas`, {
  ...
  }, [searchQuery, selectedCity, debouncedSearch]);
  ...
  const handleSearchResultSelect = (area: string) => {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 mt-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Area <span className="text-xs text-gray-500">(Please Select Emirate & City)</span>
          </label>
            ...
            <div className="relative">
              ...
              >
                ...
                <SelectContent className="bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-y-auto">
                  ...
                  {/* Search results or default areas */}
                  {isSearching ? (
                    ...
                    searchResults.length > 0 ? (
                      searchResults.map((area) => (
                        <SelectItem
                          ...
                          value={area.name}
                          className="px-4 py-2 text-sm text-gray-700 hover:bg-emerald-100 focus:bg-emerald-100 cursor-pointer"
                          onClick={() => handleSearchResultSelect(area.name)}
                        >
                          {area.name}
                        </SelectItem>
                      ))
